...
Vue.component('tabs-nav', {
  ...
  props: {
    ...
    lineWidth: { // 底部条宽度，默认单位 px，没设置则自动计算
      type: String,
      default: 'auto'
    }
  },
  ...
  data: function () {
    return {
      ...
      tabs_ele: [], // 对应每个导航的节点
      lineWidthPX: 0, // lineWidth 转换为px后对应的数值
      line_width: 0, // 导航滑块 width
      line_left: 0, // 导航滑块 left
    }
  },
  watch: {
  ...
  mounted() {
    ...
    this.lineWidthPX = this.companyTopx(this.lineWidth)
  ...
  methods: {
    ...
    setLineStyle: function (index) {
      ...
      if (this.lineWidth === 'auto') {
        ...
        this.line_width = this.lineWidthPX
        this.line_left = ele.offsetLeft + (ele.clientWidth - this.lineWidthPX) / 2
      }
    },
    // 未知单位转px
    companyTopx(company) {
      ...
      eDiv.style.width = company
      eDiv.style.height = company
...
Vue.component('top-filter', {
  ...
  methods: {
    ...
    selectOptions: function (sel, item, index) {
      ...
      this.$emit('change', this.select, index, sel)
    },
    reset: function () {
