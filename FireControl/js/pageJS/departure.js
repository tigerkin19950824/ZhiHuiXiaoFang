...
function okButFn(a){
	...
	if(a==0){
		...
		$('.eventStatus .mui-btn.Selection').each(function(index,element){
			alarmstates+=$(element).attr('alarmstate')+','
		})
	...
	// console.log('isAudit='+isAudit,'isfalse='+isfalse,'svtype='+svtype)
	GetAlarmPageList()
}
...
function GetAlarmPageList(){
	dataAjax={
		...
		starttime:starttime,
		endtime:endtime,
		alarmstate:alarmstate//告警类型
	}
	commonAjax(dataAjax,'api/Event/GetLeaveAlarmPageList',GetAlarmPageListsuccFn,'get')
}
function GetAlarmPageListsuccFn (data){
	...
	if(data.rows!=null && data.rows.length!=0){
		...
		for(var i=0 ; data.rows.length>i;i++){
			...
			datalis+='<div class="mui-card" id="'+dataRows.id+'"><div class="mui-card-header">'+
					'<div><p>'+dataRows.unitName+'<span>'+timestapFn(dataRows.creationTime)+'</span></p>'+
...
mui('.mui-table-view-chevron').on('tap','.mui-card',function(){
	window.location.href="../Home/details/StaffLeave.html?id="+$(this).attr('id');
})
