...
function FamilyAjax (datas,url,successFn,type){
	$.ajax({  
		...
		beforeSend: function (XMLHttpRequest) {
			XMLHttpRequest.setRequestHeader("Authorization", toke);
		},
		success: successFn,
		error:function(){
			console.log('请求失败！');
...
$(".search-box input").on('input propertychange',function(){
	if($(this).val()==""){
		$(".search-box .del-icon").hide();
	}else{
		$(".search-box .del-icon").show()
	}
})
